...
Communitas uses a **modular CRDT architecture** where each entity (group, channel, project, organization) is composed of multiple specialized CRDT documents, each handling a specific concern. This approach provides:
...
**Virtual Disks (Full File Replication):**
...
Every entity in Communitas is represented by multiple CRDT documents, each handling a specific concern.
...
**Example IDs:**
- `group:ocean-forest-moon-star:core`
- `group:ocean-forest-moon-star:chat`
- `channel:bright-river-wind-star:core`
- `channel:bright-river-wind-star:chat`
- `project:gentle-wave-moon-fire:core`
- `project:gentle-wave-moon-fire:chat`
- `project:gentle-wave-moon-fire:kanban`
- `project:gentle-wave-moon-fire:issues`
...
**Tombstones are pruned immediately when all active members have acknowledged them** - no periodic cleanup tasks.
...
```rust
// communitas-desktop/src/member_manager.rs
...
**Worst case:** 1000 tombstones × 100 members = ~2.2MB overhead (rare)

## Integration with Existing Systems

### Virtual Disks

**CRDT documents are separate from virtual disk files:**

```
Entity: group:ocean-forest-moon-star

CRDT Documents:
  group:ocean-forest-moon-star:core    → Members, metadata
  group:ocean-forest-moon-star:chat    → Chat messages
...
**Attachments bridge both systems:**
...
**New commands for member management:**
...
- **libSQL/Turso:** https://turso.tech/libsql
- **Four-Word Networking:** Internal crate documentation
