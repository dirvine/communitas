...
impl CoreContext {
    ...
    async fn setup_entity_message_handler(&self) -> Result<(), String> {
        ...
        let handler: crate::gossip::EntityMessageHandler = Arc::new(move |entity_id, sender_peer_id, message_bytes| {
                ...
                Ok(crdt_message) => {
                    ...
                    tokio::spawn(async move {
                        match message_service.receive_message(crdt_message).await {
                            Ok(result) => {
                                if result.accepted {
                                    info!("Stored incoming message for entity {}", entity_id_clone);
