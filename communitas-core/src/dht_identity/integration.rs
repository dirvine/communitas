...
pub async fn create_complete_identity_example() -> Result<ResolvedIdentity, String> {
    ...
    let descriptor_cid = descriptor.content_address()
    ...
    let final_descriptor_cid = descriptor.content_address()
    ...
    let connection_cid = connection_blob.content_address()
    ...
    let site_cid = site_manifest.content_address()
...
pub async fn demonstrate_identity_lifecycle() -> Result<(), String> {
    ...
    println!("   Display name: {}", identity.descriptor.preferred_display_name);
    ...
    if let Some((site_record, site_manifest)) = &identity.site_info {
        ...
        println!("   Total website content: {} bytes", site_manifest.total_size());
    }
...
mod tests {
    ...
    async fn test_complete_identity_creation() {
        ...
        assert_eq!(identity.descriptor.preferred_display_name, "Ocean Forest Moon Star");
        ...
        if let Some((conn_record, conn_blob)) = &identity.connection_info {
            ...
            assert_eq!(conn_blob.rendezvous_nodes[0].address, "bootstrap1.communitas.network:9000");
        }
    ...
    async fn test_dht_record_size_compliance() {
        ...
        let root_size = identity.root_record.to_cbor().unwrap().len();
        assert!(root_size <= super::super::DHT_RECORD_MAX_SIZE, 
                "Root record too large: {} bytes", root_size);

        if let Some((conn_record, _)) = &identity.connection_info {
            let conn_size = conn_record.to_cbor().unwrap().len();
            assert!(conn_size <= super::super::DHT_RECORD_MAX_SIZE,
                    "Connection record too large: {} bytes", conn_size);
        }

        if let Some((site_record, _)) = &identity.site_info {
            let site_size = site_record.to_cbor().unwrap().len();
            assert!(site_size <= super::super::DHT_RECORD_MAX_SIZE,
                    "Site record too large: {} bytes", site_size);
        }

        println!("✅ All DHT records within 512B limit");
        println!("   Root: {} bytes", root_size);
        if let Some((conn_record, _)) = &identity.connection_info {
            println!("   Connection: {} bytes", conn_record.to_cbor().unwrap().len());
        }
        if let Some((site_record, _)) = &identity.site_info {
