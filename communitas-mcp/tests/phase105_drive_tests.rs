...
impl TestNode {
    ...
    async fn call_tool(&self, tool: &str, params: serde_json::Value) -> bool {
        let client = reqwest::Client::new();
        let response = client
...
async fn test_drive_list_disks() {
    ...
    assert!(node.call_tool("list_disks", json!({})).await);
}
...
async fn test_drive_get_disk_stats() {
    ...
    assert!(
        node.call_tool("get_disk_stats", json!({"disk_id": "test"}))
            .await
    );
}
...
async fn test_get_upload_progress() {
    ...
    assert!(
        node.call_tool("get_upload_progress", json!({"upload_id": "test"}))
            .await
    );
}
...
async fn test_cancel_upload() {
    ...
    assert!(
        node.call_tool("cancel_upload", json!({"upload_id": "test"}))
            .await
    );
}
...
async fn test_get_download_progress() {
    ...
    assert!(
        node.call_tool("get_download_progress", json!({"download_id": "test"}))
            .await
    );
}
...
async fn test_cancel_download() {
    ...
    assert!(
        node.call_tool("cancel_download", json!({"download_id": "test"}))
            .await
    );
}
...
async fn test_create_share_link() {
    ...
    assert!(
        node.call_tool(
            "create_share_link",
            json!({"disk_id": "test", "file_path": "/f.txt", "expiration": "2026-02-29T00:00:00Z"})
        )
        .await
    );
}
...
async fn test_list_share_links() {
    ...
    assert!(
        node.call_tool("list_share_links", json!({"disk_id": "test"}))
            .await
    );
}
...
async fn test_get_staged_upload() {
    ...
    assert!(
        node.call_tool("get_staged_upload", json!({"staged_id": "test"}))
            .await
    );
}
...
async fn test_list_staged_uploads() {
    ...
    assert!(
        node.call_tool("list_staged_uploads", json!({"disk_id": "test"}))
            .await
    );
}
...
async fn test_remove_staged_upload() {
    ...
    assert!(
        node.call_tool("remove_staged_upload", json!({"staged_id": "test"}))
            .await
    );
}
...
async fn test_retry_staged_upload() {
    ...
    assert!(
        node.call_tool("retry_staged_upload", json!({"staged_id": "test"}))
            .await
    );
}
...
async fn test_get_staging_status() {
    ...
    assert!(
        node.call_tool("get_staging_status", json!({"disk_id": "test"}))
            .await
    );
}
...
async fn test_sync_staging_queue() {
    ...
    assert!(
        node.call_tool("sync_staging_queue", json!({"disk_id": "test"}))
            .await
    );
}
...
async fn test_private_disk() {
    ...
    assert!(
        node.call_tool("private", json!({"entity_id": "test", "disk_id": "test"}))
            .await
    );
}
...
async fn test_public_disk() {
    ...
    assert!(
        node.call_tool("public", json!({"entity_id": "test", "disk_id": "test"}))
            .await
    );
}
...
async fn test_canvas_add_image() {
    ...
    assert!(
        node.call_tool(
            "canvas_add_image",
            json!({"canvas_id": "test", "image_url": "http://example.com/img.png", "x": 0.0, "y": 0.0, "width": 100.0, "height": 100.0})
        )
        .await
    );
}
...
async fn test_canvas_deselect_all() {
    ...
    assert!(
        node.call_tool("canvas_deselect_all", json!({"canvas_id": "test"}))
            .await
    );
}
...
async fn test_canvas_clear() {
    ...
    assert!(
        node.call_tool("canvas_clear", json!({"canvas_id": "test"}))
            .await
    );
}
...
async fn test_canvas_undo() {
    ...
    assert!(
        node.call_tool("canvas_undo", json!({"canvas_id": "test"}))
            .await
    );
}
...
async fn test_canvas_redo() {
    ...
    assert!(
        node.call_tool("canvas_redo", json!({"canvas_id": "test"}))
            .await
    );
}
...
async fn test_canvas_get_snapshot() {
    ...
    assert!(
        node.call_tool("canvas_get_snapshot", json!({"canvas_id": "test"}))
            .await
    );
}
...
async fn test_canvas_get_remote_cursors() {
    ...
    assert!(
        node.call_tool("canvas_get_remote_cursors", json!({"canvas_id": "test"}))
            .await
    );
}
...
async fn test_canvas_flush_offline_queue() {
    ...
    assert!(
        node.call_tool("canvas_flush_offline_queue", json!({"canvas_id": "test"}))
            .await
    );
}
