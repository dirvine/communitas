...
pub async fn core_group_create(
    ...
    // All gossip operations must be within the guard scope
    {
        ...
        let entity_message = format!(
            "group:{}:{}",
            group_id,
            String::from_utf8_lossy(&metadata_bytes)
        );
        if let Err(e) = gossip_ctx
            .store_message(entity_message.as_bytes().to_vec())
            .await
        {
...
pub async fn core_group_add_member(
    ...
    let result = {
        ...
        let current_metadata = {
            ...
            let group_messages: Vec<Vec<u8>> = messages
                .iter()
                .filter(|msg| {
        ...
        // Update members list
        let mut members = current_metadata["members"]
            .as_array()
        ...
        if let Err(e) = gossip_ctx
            .publish_to_entity(&format!("group_events:{}", group_id), event_bytes)
            .await
        {
