...
async fn test_crdt_persistence_across_restart() {
    ...
    // Phase 1: Create data in first instance
    {
        let core = communitas_core::CoreContext::initialize(
            "test-user-alpha-beta-gamma".to_string(),
            "Test User".to_string(),
        ...
        let website_doc = yrs::Doc::new();
        {
            ...
            root.insert(&mut txn, "entity_id", channel.id.clone());
        ...
        println!("Created website for entity: {}", channel.id);
        ...
        let doc_id = format!("{}:private:{}", channel.id, path_hash);

        core.crdt_manager
            .save_document(&doc_id, "file", &channel.id, &file_doc)
            .await
    ...
    // Phase 2: Load data in second instance (restart simulation)
    {
        ...
        assert!(!channels.is_empty(), "No channels found after restart!");
        let channel = &channels[0];
        println!("Found channel after restart: {} ({})", channel.name, channel.id);
        assert_eq!(channel.name, "Test Channel");

        // Verify website exists
        let website_doc = core
            .crdt_manager
            .load_document(&format!("website:{}", channel.id))
            .await
...
/// and verifies that CRDT documents can be merged between them.
#[tokio::test]
async fn test_multi_instance_crdt_sync() {
    ...
    // Create a shared entity ID for testing
    let test_entity_id = uuid::Uuid::new_v4().to_string();

    // Phase 1: Instance 1 creates a channel
    let channel_name = {
        let core1 = communitas_core::CoreContext::initialize(
            "instance-one-alpha-beta".to_string(),
    ...
    // Phase 2: Instance 2 creates a different channel
    {
        let core2 = communitas_core::CoreContext::initialize(
            "instance-two-gamma-delta".to_string(),
    ...
    println!("\n📡 Simulating CRDT sync between instances...");

    // Copy instance 1 CRDTs to instance 2
    let instance1_crdt_dir = format!("{}/crdt/channel", instance1_dir);
    let instance2_crdt_dir = format!("{}/crdt/channel", instance2_dir);

    if let Ok(entries) = std::fs::read_dir(&instance1_crdt_dir) {
        for entry in entries.flatten() {
            let file_name = entry.file_name();
            let source = entry.path();
            let dest = format!("{}/{}", instance2_crdt_dir, file_name.to_string_lossy());

            // Only copy if file doesn't exist in instance 2 (simulating merge)
    ...
    // Copy instance 2 CRDTs to instance 1
    if let Ok(entries) = std::fs::read_dir(&instance2_crdt_dir) {
        for entry in entries.flatten() {
            let file_name = entry.file_name();
            let source = entry.path();
            let dest = format!("{}/{}", instance1_crdt_dir, file_name.to_string_lossy());
    ...
    // Phase 4: Verify both instances now see both channels
    {
        let core1 = communitas_core::CoreContext::initialize(
            "instance-one-alpha-beta".to_string(),
            "Instance One".to_string(),
        ...
        let channels = core1
            ...
            .list_entities()
            .await
            .expect("Failed to list channels in instance 1");
        ...
        assert_eq!(
            channels.len(),
            2,
            "Instance 1 should see both channels after sync"
        );
    }

    {
        ...
        let channels = core2
            ...
            .list_entities()
            .await
            .expect("Failed to list channels in instance 2");
        ...
        assert_eq!(
            channels.len(),
            2,
            "Instance 2 should see both channels after sync"
        );
    }
