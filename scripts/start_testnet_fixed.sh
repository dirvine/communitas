...
echo "Starting Communitas testnet with fixed configuration..."
...
TESTNET_DIR="/tmp/communitas-testnet-fixed"
rm -rf $TESTNET_DIR
mkdir -p $TESTNET_DIR

# Start nodes with proper configuration
echo "Starting node 0 (bootstrap)..."
cargo run --release -p communitas-headless \
    -- --listen 127.0.0.1:9000 \
    --config /tmp/testnet_config.toml \
