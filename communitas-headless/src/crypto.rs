...
pub fn sign_mldsa87(sk: &[u8], message: &[u8]) -> Result<Vec<u8>> {
    ...
    let sk_array: [u8; 4627] = sk.try_into()
        .map_err(|_| anyhow::anyhow!("Invalid private key length: expected 4627 bytes, got {}", sk.len()))?;
...
pub fn verify_mldsa87(pk: &[u8], message: &[u8], signature: &[u8]) -> Result<bool> {
    ...
    let pk_array: [u8; 2592] = pk.try_into()
        .map_err(|_| anyhow::anyhow!("Invalid public key length: expected 2592 bytes, got {}", pk.len()))?;
    
    let public_key = PublicKey::try_from_bytes(pk_array)
        .map_err(|e| anyhow::anyhow!("Failed to deserialize ML-DSA-87 public key: {}", e))?;
    
    // Deserialize signature
    let sig_array: [u8; 4595] = signature.try_into()
        .map_err(|_| anyhow::anyhow!("Invalid signature length: expected 4595 bytes, got {}", signature.len()))?;
...
pub fn save_keys_to_keystore(identity: &str, pk: &[u8], sk: &[u8]) -> Result<()> {
    ...
    // Save to keyring
    pk_entry.set_password(&pk_b64)
        .context("Failed to save public key to keyring")?;
    
    sk_entry.set_password(&sk_b64)
...
pub fn load_keys_from_keystore(identity: &str) -> Result<(Vec<u8>, Vec<u8>)> {
    ...
    // Load from keyring
    let mut pk_b64 = pk_entry.get_password()
        .context(format!("Failed to load public key for identity: {}", identity))?;
    
    let mut sk_b64 = sk_entry.get_password()
        .context(format!("Failed to load private key for identity: {}", identity))?;
    
    // Decode from base64
    let pk = BASE64.decode(&pk_b64)
        .context("Failed to decode public key from base64")?;
    
    let sk = BASE64.decode(&sk_b64)
    ...
    if sk.len() != 4627 {
        anyhow::bail!("Invalid private key length: expected 4627, got {}", sk.len());
    }
    
    tracing::info!("Loaded ML-DSA-87 keys from keyring for identity: {}", identity);
