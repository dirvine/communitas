...
///
/// Note: [vaultKey] is an internal storage identifier (derived from key material).
/// It is NOT for display to users. Only [displayName] should be shown in UI.
/// User identity is the ML-DSA-87 public key (pubkeyHex), available after login.
class VaultInfo {
  final String id;

  /// Internal vault storage key - NOT for display. Use [displayName] in UI.
  final String vaultKey;

  /// User-chosen display name - this is what users see.
  final String displayName;
  ...
  /// @deprecated Use [vaultKey] instead. This getter exists for compatibility.
  String get fourWords => vaultKey;
}
...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  ///
  /// Reads recent_identities from app_config.json to get display names.
  /// The display_name is stored in the config for listing without decrypting vaults.
  Future<void> _loadVaults() async {
    try {
      final storagePath = await _getStoragePath();
      final configFile = File('$storagePath/app_config.json');
      ...
      if (await configFile.exists()) {
        ...
        try {
          ...
          final recentIdentities = config['recent_identities'] as List<dynamic>?;

          if (recentIdentities != null) {
            for (final identity in recentIdentities) {
              final map = identity as Map<String, dynamic>;
              final fourWords = map['four_words'] as String? ?? '';
              final displayName = map['display_name'] as String? ?? fourWords;
              final lastUsed = map['last_used'] as int? ?? 0;

              if (fourWords.isNotEmpty) {
                // Verify the vault directory actually exists before listing it
                final vaultDir = Directory('$storagePath/vaults/$fourWords');
                if (await vaultDir.exists()) {
                  vaults.add(VaultInfo(
                    id: fourWords,
                    vaultKey: fourWords,
                    displayName: displayName,
                    createdAt: DateTime.fromMillisecondsSinceEpoch(lastUsed * 1000),
                    lastUsed: DateTime.fromMillisecondsSinceEpoch(lastUsed * 1000),
                  ));
                } else {
                  debugPrint('Skipping stale vault entry: $fourWords (directory not found)');
                }
              }
            }
          }
        } catch (e) {
          debugPrint('Error reading app config: $e');
        }
