...
impl SiteManifestRecord {
    ...
    pub fn new(sequence: u32, timestamp: u64, site_cid: ContentId, ttl_seconds: u32) -> Self {
        Self {
            version: PROTOCOL_VERSION,
            sequence,
            timestamp,
...
mod tests {
    ...
    fn test_identity_root_record_cbor_serialization() {
        ...
        let deserialized = IdentityRootRecord::from_cbor(&cbor_bytes).expect("Should deserialize");
    ...
    fn test_connection_record_operations() {
        ...
        let cbor_bytes = record.to_cbor().expect("Should serialize");
        let deserialized = ConnectionRecord::from_cbor(&cbor_bytes).expect("Should deserialize");
        assert_eq!(record, deserialized);
    ...
    fn test_site_manifest_record_operations() {
        ...
        // Test serialization round-trip
        let cbor_bytes = record.to_cbor().expect("Should serialize");
        let deserialized = SiteManifestRecord::from_cbor(&cbor_bytes).expect("Should deserialize");
        assert_eq!(record, deserialized);
    ...
    fn test_records_with_optional_fields() {
        ...
        record
            .validate_size()
            .expect("Should be within limits even with optional fields");
        ...
        let cbor_size = cbor_bytes.len();
        println!(
            "IdentityRootRecord with optional fields CBOR size: {} bytes",
            cbor_size
        );
        assert!(cbor_size <= super::super::DHT_RECORD_MAX_SIZE);
    }
