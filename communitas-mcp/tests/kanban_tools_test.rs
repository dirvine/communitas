...
async fn test_list_kanban_boards() {
    ...
    if let Some(parsed) = &r.parsed {
        if let Some(boards) = parsed.get("boards").and_then(|v| v.as_array()) {
...
async fn test_list_kanban_columns() {
    ...
    if let Some(parsed) = &r.parsed {
        if let Some(columns) = parsed.get("columns").and_then(|v| v.as_array()) {
            assert!(
                !columns.is_empty(),
...
async fn test_list_kanban_cards() {
    ...
    if let Some(parsed) = &r.parsed {
        if let Some(cards) = parsed.get("cards").and_then(|v| v.as_array()) {
