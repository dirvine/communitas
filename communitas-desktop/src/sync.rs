...
pub async fn sync_start_tip_watcher(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if let Err(e) = app_handle.emit("sync-progress", &serde_json::json!({
                "phase": "anti_entropy",
                "status": "running",
...
pub async fn sync_fetch_deltas(
    ...
    let result = {
        ...
        match gossip_ctx.find_contact(&peer_addr).await {
            Ok(peer_id) => {
                debug!("Resolved peer {} via contact lookup: {:?}", peer_addr, peer_id);
                
                // Emit sync progress event
                if let Err(e) = app.emit("sync-progress", &serde_json::json!({
                    "phase": "manual_fetch",
            ...
            Err(e) => {
                ...
                // Emit error event
                if let Err(e) = app.emit("sync-error", &serde_json::json!({
                    "phase": "manual_fetch",
                    "peer": peer_addr,
                    "error": e.to_string(),
                    "timestamp": chrono::Utc::now().to_rfc3339()
...
pub async fn sync_get_status(
    ...
    let message_count = {
        let guard = gossip_state.read().await;
        let gossip_ctx = guard.as_ref()
            .ok_or("GossipContext not initialized")?;
        
        gossip_ctx.get_all_messages().await
            .map(|msgs| msgs.len())
            .unwrap_or(0)
    };
