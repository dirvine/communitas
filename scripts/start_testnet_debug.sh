...
echo "Starting Communitas testnet with debug build (faster)..."
...
TESTNET_DIR="/tmp/communitas-testnet-debug"
rm -rf $TESTNET_DIR
mkdir -p $TESTNET_DIR

# Start nodes with debug build (much faster)
echo "Starting node 0 (bootstrap)..."
cargo run -p communitas-headless \
