...
async fn test_vault_creation_non_blocking() -> Result<()> {
    ...
    let vault_task = tokio::spawn(async move {
        ...
        let storage_manager = EncryptedStorageManager::new(config).await?;
        storage_manager.create_vault("test-async-ocean-forest", "test-password", "Test User").await
    });
    ...
    assert!(!vault_id.is_empty());
    assert!(duration < Duration::from_secs(10), "Vault creation too slow: {:?}", duration);
    
    println!("✅ Vault created in {:?} with {} UI updates", duration, ui_updates);
    Ok(())
}
...
async fn test_keyring_disabled() -> Result<()> {
    ...
    assert!(!vault_id.is_empty(), "Vault creation without keyring failed");
...
async fn test_pbkdf2_performance() -> Result<()> {
    ...
    for (iterations, max_duration) in test_cases {
        ...
        assert!(duration < max_duration, 
               ...
               iterations, duration, max_duration);
...
async fn test_concurrent_operations() -> Result<()> {
    ...
    for i in 0..3 {
        ...
        let task = tokio::spawn(async move {
            ...
            storage_manager.create_vault(&identity, "test-password", &display_name).await
        });
        tasks.push(task);
    }
    ...
    for result in results {
        assert!(result.is_ok(), "Concurrent vault creation failed: {:?}", result);
    }
...
async fn test_timeout_behavior() -> Result<()> {
    ...
    assert!(duration < Duration::from_secs(60), "Operation should complete before timeout");
    
    println!("✅ Timeout mechanism working correctly, completed in {:?}", duration);
    Ok(())
}