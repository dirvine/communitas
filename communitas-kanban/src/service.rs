...
impl KanbanService {
    ...
    // Compile-time assertion: MAX_BOARD_DOCUMENTS (64) is non-zero.
    // The panic branch is unreachable at runtime - it's a const evaluation guard.
    #[allow(clippy::panic)]
    const LRU_CAPACITY: NonZeroUsize = match NonZeroUsize::new(MAX_BOARD_DOCUMENTS) {
        Some(n) => n,
        None => core::panic!("MAX_BOARD_DOCUMENTS must be non-zero"), // CONST_SAFE
    };
