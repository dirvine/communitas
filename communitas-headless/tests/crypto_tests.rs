...
fn test_keygen_produces_valid_mldsa87_keys() {
    ...
    assert!(sk.iter().any(|&b| b != 0), "Private key should not be all zeros");
}
...
fn test_sign_verify_roundtrip() {
    ...
    assert_eq!(signature.len(), 4595, "ML-DSA-87 signature should be 4595 bytes");
...
fn test_verify_fails_with_wrong_message() {
    ...
    assert!(!verify_result.unwrap(), "Signature should NOT verify with wrong message");
}
...
fn test_verify_fails_with_wrong_key() {
    ...
    assert!(verify_result.is_ok());
    assert!(!verify_result.unwrap(), "Signature should NOT verify with wrong key");
}
...
fn test_keystore_persistence() {
    ...
    crypto::save_keys_to_keystore(identity, &pk, &sk)
        .expect("Saving keys should succeed");
    
    // Load keys from keystore
    let (loaded_pk, loaded_sk) = crypto::load_keys_from_keystore(identity)
        .expect("Loading keys should succeed");
...
fn test_keystore_roundtrip_with_signing() {
    ...
    let signature = crypto::sign_mldsa87(&loaded_sk, message).expect("Signing with loaded key failed");
    let verified = crypto::verify_mldsa87(&loaded_pk, message, &signature).expect("Verification failed");
